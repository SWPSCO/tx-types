...
fn test_compare_implementations() {
    ...
    let expected = [0x2fu8, 0x19, 0x05, 0x84, 0x78, 0x33, 0x73, 0x89, 0x6d, 0x86, 0x40, 0x0d, 0xb6, 0x0c, 0x49, 0x5d, 0xa9, 0xa2, 0xec, 0xd4, 0x79, 0x96, 0x66, 0xb1, 0x0f, 0xbf, 0x3d, 0x72, 0x43, 0xfe, 0xf9, 0x2d];
        ...
        if manual_result != biguint_result {
            ...
            for i in 0..32 {
                if manual_result[i] != biguint_result[i] {
                    println!("  Byte {}: manual={:02x} biguint={:02x}", i, manual_result[i], biguint_result[i]);
                }
            }
    ...
    assert_eq!(manual_result, expected, "Manual implementation should match expected");
}
