...
mod tests {
    ...
    use crate::transaction_types::{Hash, Lock, SchnorrPubkey, F6LT};
    ...
    fn test_lock_base58_roundtrip() {
        ...
        assert_eq!(
            lock.pubkeys.len(),
            decoded.pubkeys.len(),
            "Should have same number of pubkeys"
        );

        // Check each pubkey is present
        for pk in lock.pubkeys.iter() {
            assert!(
                decoded.pubkeys.iter().any(|dpk| dpk == pk),
                "Pubkey should be present in decoded lock"
            );
        }
    }
