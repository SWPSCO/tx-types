...
use crate::crypto::utils::{
    add_mod_n, be32_atom_to_t8_le, be32_lt, is_zero32, mul_mod_n, t8_to_be32,
    trunc_g_order_to_be32, CHEETAH_N,
};
use crate::hashing::hasher::hash_transcript_list;
use crate::transaction_types::{
    Chal, Coins, Hash, Inputs, NName, RawTransaction, SchnorrPubkey, SchnorrSignature, Sig,
    Signature, Spend, T8,
};
use crate::transaction_types_v0::{InputV0, InputsV0, SpendV0};
...
pub fn sign_spend(spend: SpendV0, secret_key: T8, public_key: SchnorrPubkey) -> (T8, T8) {
...
mod tests {
    ...
    fn test_sign_tx_with_known_good_data() {
        ...
        assert!(
            all_signed,
            "All inputs should have signatures after signing"
        );

        // 2. The transaction ID changed from the unsigned version
        assert_ne!(
            signed_tx.id.values, unsigned_tx.id.values,
        ...
        assert_eq!(
            signed_tx.id.values, expected_signed_tx.id.values,
