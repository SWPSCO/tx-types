...
use crate::transaction_types::{Coins, Input, Inputs, NNote, Seeds, Signature, Spend};
use crate::transaction_types_v0::{InputV0, InputsV0, NNoteV0, SeedV0, SeedsV0, SpendV0};
...
pub fn calculate_timelock_range(inputs: &[InputV0]) -> crate::transaction_types::TimelockRange {
    use crate::transaction_types::{PageNumber, TimelockRange};
...
pub fn create_raw_transaction_noun(input_list: Vec<InputV0>) -> NounSlab {
    ...
    let inputs = Inputs::V0(InputsV0 {
        p: inputs_zmap.clone(),
    });

    // Step 2: Calculate total fees by summing all input fees
    let total_fees = Coins {
        value: input_list.iter().map(|input| input.spend.fee.value).sum(),
    };
    ...
    let raw_tx_without_id = T(&mut slab, &[inputs_noun, timelock_noun, total_fees_noun]);
...
mod tests {
    ...
    fn test_single_input_hoon_generator_exact() {
        ...
        println!(
            "TX-ID from compute_tx_id:   {:016x?}",
            computed_tx_id.values
        );
        println!(
            "Expected TX-ID from Hoon:    {:016x?}",
            expected_tx_id.values
        );

        // Verify the tx_id matches
        assert_eq!(
            computed_tx_id, expected_tx_id,
            "TX-ID should match Hoon generator output"
        );
        assert_eq!(
            tx_id_values, expected_tx_id.values,
            "TX-ID from noun should match expected"
        );
    }
    ...
    fn test_single_input_noun_structure_exact() {
        ...
        let tx_id_1 = tx_id_cell
            .head()
            .as_atom()
            .expect("First tx-id should be atom");
        assert_eq!(
            tx_id_1.as_u64().unwrap(),
            13077428501917685021,
            "First TX-ID value"
        );
                        ...
                        if let Ok(inner4) = inner3.tail().as_cell() {
                            if let Ok(inner5) = inner4.tail().as_cell() {
                                ...
                                if let Ok(assets_atom) = inner5.tail().as_atom() {
                                    assert_eq!(
                                        assets_atom.as_u64().unwrap(),
                                        100,
                                        "Assets should be 100"
                                    );
                                }
                            }
                        }
        ...
        fn find_value_in_noun(noun: Noun, target: u64) -> bool {
            match noun.as_atom() {
                ...
                Err(_) => {
                    if let Ok(cell) = noun.as_cell() {
                        find_value_in_noun(cell.head(), target)
                            || find_value_in_noun(cell.tail(), target)
                    } else {
                        false
                    }
                }
        ...
        assert!(
            find_value_in_noun(*root, 90),
            "Structure should contain gift value of 90"
        );
        assert!(
            find_value_in_noun(*root, 100),
            "Structure should contain assets value of 100"
        );
        assert!(
            find_value_in_noun(*root, 10),
            "Structure should contain fee value of 10"
        );
    ...
    fn test_spend_structure_debug() {
        ...
        if let Ok(cell) = spend_noun.as_cell() {
            ...
            println!(
                "Head (signature): is_atom={}, is_cell={}",
                head.as_atom().is_ok(),
                head.as_cell().is_ok()
            );
            ...
            if let Ok(tail_cell) = tail.as_cell() {
                ...
                println!(
                    "  Tail.head (seeds): is_atom={}, is_cell={}",
                    tail_head.as_atom().is_ok(),
                    tail_head.as_cell().is_ok()
                );
                println!(
                    "  Tail.tail (fee): is_atom={}, is_cell={}",
    ...
    fn test_two_inputs_from_hoon_generator() {
                ...
                map: {
                    ...
                    let schnorr_sig = SchnorrSignature {
                        chal: Chal {
                            values: T8 {
                                values: [
                                    0x8201da19, 0xcde16f4c, 0x8578369d, 0x23d0776, 0x7ed21f2c,
                                    0x8e8d0348, 0x98e0649f, 0x5f5e4365,
                                ],
                            },
                        },
                        sig: Sig {
                            values: T8 {
                                values: [
                                    0xb3a46cd6, 0xd5a05703, 0x9910e837, 0xcbf4da5b, 0x5bd6b444,
                                    0x1840c16b, 0xfe99b35b, 0x4481d614,
                                ],
                            },
                        },
                    };
                ...
                map: {
                    ...
                    let schnorr_sig = SchnorrSignature {
                        chal: Chal {
                            values: T8 {
                                values: [
                                    0xb8178004, 0x5745a0dd, 0x7e0d800c, 0x84a90cfb, 0xd687cd39,
                                    0x78103bf1, 0xa51874d5, 0x0f9e0a44,
                                ],
                            },
                        },
                        sig: Sig {
                            values: T8 {
                                values: [
                                    0x420519fc, 0xafba7efe, 0x4797d97f, 0x840b45d5, 0xaf311e38,
                                    0x96387987, 0xd712d047, 0x5d8a8b93,
                                ],
                            },
                        },
                    };
        ...
        assert_eq!(
            computed_tx_id, expected_tx_id,
            "TX-ID should match expected from Hoon"
        );
        assert_eq!(
            tx_id_values, expected_tx_id.values,
            "Extracted TX-ID should match expected from Hoon"
        );
    }
    ...
    fn test_three_input_zmap_hash() {
        ...
        assert_eq!(
            ...
            "Three input ZMap hash mismatch!"
        );
