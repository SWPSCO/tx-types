...
use super::hasher::{digest_to_base58, hash_hashable};
use crate::transaction_types::{Coins, Hash, Inputs, TimelockRange};
...
///   leaf+total-fees.raw
/// ```
pub fn compute_tx_id(inputs: &Inputs, timelock_range: &TimelockRange, total_fees: Coins) -> Hash {
    // Build the hashable structure as a triple
    let inputs_hashable = match inputs {
        Inputs::V0(v0) => {
            v0.p.to_hashable(|nname| nname.to_hashable(), |input| input.to_hashable())
        }
...
mod tests {
    ...
    fn test_compute_tx_id() {
        ...
        use crate::transaction_types::{
            Coins, Input, InputV0, Inputs, InputsV0, Lock, NName, NNote, NNoteHead, NNoteV0,
            PageNumber, SeedV0, Seeds, SeedsV0, Source, Spend, SpendV0, Timelock,
        };
    ...
    fn test_nnote_to_hashable_matches_hoon() {
        ...
        assert_eq!(
            hash_via_method, expected_hash,
            "NNote.to_hash() should match Hoon"
        );
    ...
    fn test_nnote_999_to_hashable_matches_hoon() {
        ...
        println!(
            "\npubkey1 (x[0]={}): hash={:x?}",
            pubkey1.x.values[0], pubkey1_hash.values
        );
        ...
        println!(
            "pubkey2 (x[0]={}): hash={:x?}",
            pubkey2.x.values[0], pubkey2_hash.values
        );
        ...
        match hash1_ubig.cmp(&hash2_ubig) {
            ...
            std::cmp::Ordering::Greater => {
                println!("pubkey1 > pubkey2 (pubkey2 should come first)")
            }
            std::cmp::Ordering::Equal => {
                println!("pubkey1 == pubkey2 (would use dor_tip fallback)")
            }
        }
        ...
        match &lock_hashable {
            Hashable::Cell(m_part, pubkeys_part) => {
                println!("  m: {:?}", m_part);
                println!(
                    "  pubkeys structure type: {:?}",
                    std::mem::discriminant(pubkeys_part.as_ref())
                );
            }
        ...
        let expected_timelock_hash = Hash {
            values: [
        ...
        assert_eq!(
            ...
            "NNote (origin-page=999) hash should match Hoon"
        );
    ...
    fn test_to_hash_methods() {
        ...
        assert_eq!(
            hash_from_method, hash_from_manual,
            "NName to_hash should match manual approach"
        );
        ...
        assert_eq!(
            hash_from_method, hash_from_manual,
            "TimelockRange to_hash should match manual approach"
        );
        ...
        assert_eq!(
            hash_from_method, hash_from_manual,
            "Timelock to_hash should match manual approach"
        );
        ...
        assert_eq!(
            hash_from_method, hash_from_manual,
            "Source to_hash should match manual approach"
        );
        ...
        assert_eq!(
            hash_from_method, hash_from_manual,
            "Lock to_hash should match manual approach"
        );
        ...
        assert_eq!(
            hash_from_method, hash_from_manual,
            "Seed to_hash should match manual approach"
        );
        ...
        assert_eq!(
            hash_from_method, hash_from_manual,
            "Seeds to_hash should match manual approach"
        );
        ...
        assert_eq!(
            hash_from_method, hash_from_manual,
            "Spend to_hash should match manual approach"
        );
        ...
        assert_eq!(
            hash_from_method, hash_from_manual,
            "Input to_hash should match manual approach"
        );
        ...
        assert_eq!(
            hash_from_method, hash_from_manual,
            "NNote to_hash should match manual approach"
        );
