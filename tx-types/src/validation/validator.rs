...
impl TransactionValidator {
    ...
    pub fn validate_transaction(transaction: &RawTransaction) -> Result<(), TransactionValidationError> {
    ...
    fn validate_signatures(tx: &RawTransaction) -> Result<(), TransactionValidationError> {
        ...
        for (name, input) in inputs.p.tap() {
            ...
            let signature = input.spend.signature.as_ref()
                .ok_or_else(|| TransactionValidationError::InvalidSignature(
                    format!("Input {:?} has no signature", name)
                ))?;
            ...
            if sig_count == 0 {
                return Err(TransactionValidationError::InvalidSignature(
                    format!("Input {:?} has empty signature map", name)
                ));
            }
            ...
            if (sig_count as u64) < input.note.lock.m {
                return Err(TransactionValidationError::InvalidSignature(
                    format!("Input {:?} needs {} signatures but has {}",
            ...
            for (pubkey, schnorr_sig) in signature.map.tap() {
                ...
                if !input.note.lock.pubkeys.has(&pubkey) {
                    return Err(TransactionValidationError::InvalidSignature(
                        format!("Pubkey not authorized for input {:?}", name)
                    ));
                }
                ...
                if !schnorr_verify_digest(pubkey.clone(), sig_hash.clone(), schnorr_sig.clone()) {
                    return Err(TransactionValidationError::InvalidSignature(
                        format!("Invalid Schnorr signature for input {:?}", name)
                    ));
                }
            }
        }
    ...
    fn validate_funds(tx: &RawTransaction) -> Result<(), TransactionValidationError> {
        ...
        for (_, input) in inputs.p.tap() {
            ...
            for seed in input.spend.seeds.set.tap() {
                total_output = total_output.checked_add(seed.gift.value)
                    .ok_or_else(|| TransactionValidationError::InsufficientFunds(
            ...
            // Fee from this input
            total_output = total_output.checked_add(input.spend.fee.value)
                .ok_or_else(|| TransactionValidationError::InsufficientFunds(
                    "Fee overflow".to_string()
                ))?;
        }
        ...
        if total_input < total_output {
            return Err(TransactionValidationError::InsufficientFunds(
                format!("Inputs {} < Outputs + Fees {}", total_input, total_output)
            ));
        }
...
mod tests {
    ...
    fn test_validate_signed_transaction() {
        ...
        assert!(result.is_ok(), "Valid signed transaction should pass validation: {:?}", result.err());
    ...
    fn test_invalid_signature_modified_challenge() {
        ...
        use crate::transaction_types::{Inputs, Input, SchnorrSignature, Chal, Sig};

        // Load the pre-signed transaction
        let mut slab: NounSlab = NounSlab::new();
        let noun = slab.cue_into(SIGNED_TX_JAM.into())
            .expect("Failed to decode signed transaction JAM");
        let mut tx = RawTransaction::from_noun(&noun)
            .expect("Failed to decode RawTransaction from noun");
        ...
        for (name, input) in tx.inputs.p.tap() {
            ...
            if !modified {
                if let Some(ref sig) = new_input.spend.signature {
                    ...
                    new_input.spend.signature = Some(crate::transaction_types::Signature {
                        map: new_sig_map,
                    });
                }
            }
