...
impl F6Element {
    ...
    pub fn add(&self, other: &Self) -> Self {
        F6Element([
            ...
            self.0[3] + other.0[3], self.0[4] + other.0[4], self.0[5] + other.0[5],
        ])
    }
    ...
    pub fn neg(&self) -> Self {
        F6Element([-self.0[0], -self.0[1], -self.0[2], -self.0[3], -self.0[4], -self.0[5]])
    }
    
    /// Scalar multiplication using working implementation pattern
    pub fn mul_by_scalar(&self, s: Belt) -> Self {
        F6Element([self.0[0]*s, self.0[1]*s, self.0[2]*s, self.0[3]*s, self.0[4]*s, self.0[5]*s])
    }
