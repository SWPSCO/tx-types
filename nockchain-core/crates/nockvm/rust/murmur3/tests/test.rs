...
fn test_static_strings() {
    ...
    for test in &tests {
        assert_eq!(
            murmur3::murmur3_32(&mut str_as_cursor(test.string), 0).unwrap_or_else(|err| panic!(
        ...
        assert_eq!(
            murmur3::murmur3_32(&mut str_as_chained_cursor(test.string), 0).unwrap_or_else(
                |err| panic!(
                    "Panicked with {err:?} at {}:{} (git sha: {:?})",
                    file!(),
                    line!(),
        ...
        let hash =
            murmur3::murmur3_x86_128(&mut str_as_cursor(test.string), 0).unwrap_or_else(|_| {
                panic!(
                    "Called `expect()` at {}:{} (git sha: {})",
                    file!(),
                    line!(),
        ...
        let chained_hash = murmur3::murmur3_x86_128(&mut str_as_chained_cursor(test.string), 0)
            .unwrap_or_else(|_| {
                panic!(
                    "Called `expect()` at {}:{} (git sha: {})",
                    file!(),
        ...
        let hash =
            murmur3::murmur3_x64_128(&mut str_as_cursor(test.string), 0).unwrap_or_else(|_| {
                panic!(
                    "Called `expect()` at {}:{} (git sha: {})",
                    file!(),
                    line!(),
        ...
        let chained_hash = murmur3::murmur3_x64_128(&mut str_as_chained_cursor(test.string), 0)
            .unwrap_or_else(|_| {
                panic!(
                    "Called `expect()` at {}:{} (git sha: {})",
                    file!(),
