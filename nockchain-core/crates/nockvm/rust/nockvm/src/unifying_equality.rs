...
/// This function checks if two nouns are equal by recursively unifying them.
/// # Safety: This function is unsafe because it dereferences raw pointers and assumes that the
/// nouns are valid and properly aligned. It also assumes that the stack is in a valid state.
/// The pointer checks (acyclic, forwarding, junior) are only done when the features are toggled.
pub unsafe fn unifying_equality(stack: &mut NockStack, a: *mut Noun, b: *mut Noun) -> bool {
    ...
    // if a.as_ref_unchecked().raw_equals(b.as_ref_unchecked()) {
    if (*a).raw_equals(&*b) {
        return true;
    };
